...
def runCmdTimeout(cmd, sol, bench, size):
    ...
    assert size not in g_results[sol][bench]
    if runtime > 0:  # for mono and cobra
        g_results[sol][bench][size] = runtime
    else:  # for other bsls
        g_results[sol][bench][size] = (end - start) * 1000  # in ms



def getBenchSize(exp):
    elems = re.split("-|\.", exp)
    print(elems)
    assert len(elems) == 2 or len(elems) == 3
    return elems[0], int(elems[1])
...
def usage():
    print("Usage: run_z3_mono_cobra.py <one-shot-folder>")
